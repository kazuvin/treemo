...
例外はツリーブロックで、形が変わったときにノードの位置を滑らせる
（[tree-block.md](tree-block.md) の「動き」）。もう 1 つは編集 / 閲覧のスイッチ
（`src/app/reading-toggle.tsx`）で、選んでいる側の塗りを 1 枚にして隣へ滑らせる（160ms）。
2 つの塗りが消えて現れるより、どちらからどちらへ移ったかが一目でわかるため。
どちらも `motion-safe:` の内側で、動きを減らす設定では即座に切り替わる。
