...
- [x] **3-1 解析と書き戻し。** `parse` / `serialize`（`features/diagram/utils/`）。
...
読む: [ツリーブロック](tree-block.md)、[キー操作](keybindings.md) の DIAGRAM モード

- [x] **4-1 DIAGRAM モードと移動。** 入り方・出方、`hjkl`、`gg` / `G`、選択の表示。
...
- [x] **5-4 ヒント。** 画面の要素（`<Space>j`）と DIAGRAM モードのノード（`f`）。F-UX-3。
...
- [x] **6-2 速さの確認。** 要件定義の非機能要件の数字を測る。
  ...
  - 215 ノードのツリーで DIAGRAM モードの 1 操作: 移動 3〜8ms、入れ替え・字下げ・取り消し 4〜8ms、
...
MVP の外で足したもの。読む: [要件定義](requirements.md) の F-EDIT-5・F-EDIT-6・F-EDIT-8・F-VAULT-8、
