...
| 編集モードか閲覧モードか | `src/app/ui-store.ts` + アプリの状態として保存。エディタの中は `src/lib/reading.ts` の `StateField` | エディタとツリーの拡張が同じ値を見る必要があり、feature どうしは import できないので共有層に置く。app が ui-store の変化を effect でエディタに流す |
| DIAGRAM モード中のブロックの写し、全画面・案内・倍率の設定 | `features/diagram/stores/diagram-store.ts` | 全画面の表示が読む。正本は `StateField` の方で、拡張がここへ流す |
